...
def estimate_z_and_log_lik(clf, y_hat, data, not_nan, n_samples):
    ...
    for i in range(len(use_z_values)):
        probs[not_nan[:,i],i,:] = clf.predict_proba(data[not_nan[:,i], :][:, cols[i]])
    ...
    def generate_z_samples(k):
        ...
        while n_samples_missing > 0:
            ...
            for i in range(len(use_z_values)):
                if not_nan[k,i]:
                    Foo[:,i] = np.random.choice(clf.classes_, n_samples_missing*10, p=probs[k,i,:]).tolist()
    ...
    z_train_temp[:,:] = np.nan
    for i in range(len(use_z_values)):
        z_train_temp[not_nan[:,i], i] = clf.predict(data[not_nan[:,i],:][:, cols[i]])
    ...
    y_mapping = {clf.classes_[0]: 0, clf.classes_[1]: 1, clf.classes_[2]: 2}
    log_prob = np.zeros((len(use_z_values), 3, n))
    for i in range(len(use_z_values)):
        log_prob[i, :, not_nan[:,i]] = clf.predict_log_proba(data[not_nan[:,i],:][:, cols[i]])
...
def em_algorithm(clf, y_hat, data, not_nan, iter, dev_data, y_dev, not_nan_dev, n_samples):
    ...
    for k in range(iter):
        ...
        for i in range(len(use_z_values)):
            try:
                clf.fit(np.repeat(data[not_nan[:,i],][:,cols[i]], n_samples, axis=0), z[np.repeat(not_nan[:,i], n_samples, axis=0), i])
            except:
                models_not_updated += [i,]
            z_dev[not_nan_dev[:,i],i] = clf.predict(dev_data[not_nan_dev[:,i],][:,cols[i]])
...
if __name__ == "__main__":
    ...
    if not continue_training:
        prev_iter = 0
    prev_directory = 'vers43/MLP_Classifiers_' + str(amount_training_data) + 'k_training_' + str(prev_iter) + '_iter_' + 'NN_size_' + str_size_hidden
    folder_name = 'vers43/MLP_Classifiers_' + str(amount_training_data) + 'k_training_' + str(prev_iter + em_iter) + '_iter_' + 'NN_size_' + str_size_hidden
    ...
    if continue_training:
        ...
        with open(prev_directory + "/MLP_Classifier.pkl", "rb") as f:
            clf = pickle.load(f)
    ...
    for i in range(len(use_z_values)):
        with open(folder_name + '/MLP_Classifier.pkl','wb') as f:
            pickle.dump(clf,f)
